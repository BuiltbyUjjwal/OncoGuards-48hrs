...
import { ShieldCrossIcon, SecureShieldIcon, SpinnerIcon, AlertCircleIcon } from './assets/MedicalIcons';
...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const { sendPasswordReset, signupWithEmail } = useAuth();
  ...
  const handleModalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalType === 'forgot-password') {
        const res = await sendPasswordReset(modalEmail.trim());
        if (res.success) {
          setModalSubmitted(true);
        } else {
          setModalError(res.error || 'Unable to send reset email.');
        }
      } else {
        // Minimal fallback for create account
        const res = await signupWithEmail(modalEmail.trim(), 'Temp1234!', 'User');
        if (res.success) {
          setModalSubmitted(true);
          if (onNavigateToDashboard) {
            onNavigateToDashboard(modalEmail.trim());
          }
        } else {
          setModalError(res.error || 'Unable to create account.');
        }
      }
    } catch {
        ...
        <div className="left-inner-wrapper">
          ...
          <header className="left-header">
            ...
            <p className="login-logo-disclaimer">
              {t('auth.brandDisclaimer', 'OncoGuards gives you a risk estimate, not a diagnosis.')}
            </p>
          </header>
          ...
          <div className="left-hero-content">
            <div className="hero-pill-tag">
              ...
              <span>{t('auth.platformTag', 'Clinical AI Prevention Platform')}</span>
            </div>

            <h1 className="hero-main-heading">
              {t('auth.heroHeading', 'Early awareness saves lives.')}
            </h1>

            <p className="hero-description">
              {t('auth.heroDescription', "OncoGuard is a calm, explainable cancer risk check — built for people who don't have easy access to a specialist. It's not a diagnosis. It's a reason to act early.")}
            </p>
          </div>
          ...
          <footer className="left-footer">
            <div className="footer-security-tag">
              ...
              <span>{t('auth.securityTagline', 'HIPAA Compliant Protocol • Clinical AI Safeguards • ISO 27001 Certified')}</span>
            </div>
          </footer>
        </div>
      </section>
          ...
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            {modalSubmitted ? (
              ...
              <form onSubmit={handleModalSubmit} className="modal-form">
                {modalError && (
                  <div className="login-alert login-alert-error" role="alert" style={{ marginBottom: '16px' }}>
