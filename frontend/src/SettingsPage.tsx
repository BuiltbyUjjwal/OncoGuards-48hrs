...
import { isClerkConfigured } from './components/ClerkAuthAdapter';
...
export const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  // Real Google Auth Status
  const clerkActive = isClerkConfigured();
  const isGoogleConnected = Boolean(basicInfo.googleConnected || clerkActive);
  const googleConnectedEmail = basicInfo.googleEmail || email || 'Connected with Google SSO';

  const displayName = fullName.trim() || basicInfo.fullName?.trim() || 'Profile';
  const userInitial = displayName !== 'Profile' ? displayName.charAt(0).toUpperCase() : null;
      ...
      <main className="dashboard-main-area settings-main-canvas">
        ...
        <div className="settings-content-grid">
            ...
            <form onSubmit={handleSaveChanges} className="settings-form" noValidate>
              ...
              <div className="profile-pic-upload-container">
                ...
                <div className="profile-pic-actions">
                  ...
                  <span className="pic-hint-text">JPG, PNG, or WEBP under 2MB.</span>
                </div>
              </div>
              ...
              <div className="settings-actions-bar">
                <div className="settings-linked-actions">
                  ...
                  >
                    ...
                    <span>{t('settings.editHealthProfile', 'Edit Health Profile')}</span>
                  </button>
                </div>
              ...
              <div className="google-status-top">
                ...
                <div className="google-status-details">
                  ...
                  <span className="google-account-email">
                    {isGoogleConnected ? googleConnectedEmail : 'No Google SSO account linked'}
                  </span>
                </div>
              </div>
              <p className="google-status-note">
                {isGoogleConnected
                  ? 'Your account can authenticate seamlessly via Google Single Sign-On.'
                  : 'You are signed in via Email & Password.'}
              </p>
            </div>
          </section>
          ...
          <section className="settings-card security-overview-section" style={{ marginTop: '24px' }}>
            ...
            {/* Language Setting */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 0', borderBottom: '1px solid var(--color-border, #E2E8F0)' }}>
              <div>
                <h4 style={{ fontWeight: 600, color: 'var(--color-primary-text)', marginBottom: '4px' }}>{t('settings.language', 'Language')}</h4>
                <p style={{ color: 'var(--color-muted-text)', fontSize: '0.9rem' }}>{t('settings.languageSubtitle', 'Select your preferred interface language.')}</p>
              </div>
              <div className="language-toggle-group">
                ...
                <button
                  ...
                  className={`lang-option-btn ${currentLanguage === 'hi' ? 'active' : ''}`}
                >
                  हिंदी
                </button>
              </div>
