...
import { executeEmailSignUp, executeGoogleOAuth, isClerkConfigured } from './ClerkAuthAdapter';
...
export const SignUpForm: React.FC<SignUpFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await executeEmailSignUp({
      ...
      if (!result.success) {
        ...
        setSuccessBanner(`Account created successfully for ${result.user?.email}`);
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        setSuccessBanner(`Registered with Google as ${result.user?.email}`);
        if (onSuccess && result.user) {
          onSuccess(result.user.email);
        }
      }
  ...
  return (
    <div className="signup-card" role="region" aria-label="Create Account Card">
      {/* Card Header */}
      <div className="signup-card-header">
        <h1 className="signup-heading">Create Account</h1>
        <p className="signup-subtext">
          Join OncoGuards AI for cancer risk awareness & early screening guidance
        </p>
      </div>
